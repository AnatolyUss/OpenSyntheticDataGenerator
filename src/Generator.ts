...
export class Generator {
    ...
    public static async generate () {
        ...
        const tablesConfigurationDictionary: any = await FsOps.readSyntheticDataConfiguration(baseDir, 'synthetic_data_configuration');

        for (const tableName in tablesConfigurationDictionary) {
            //
        }

        const dbAccess: DBAccess = <DBAccess> await Generator._getQueriesInstance();
        await TableConfiguration.getTablesConstraints(dbAccess, Generator._tablesConfiguration);
